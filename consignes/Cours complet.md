---
# Modèle de consignes Transcriptor — dupliquer puis modifier pour créer ses variantes.
#
# Tout ce qui suit la seconde ligne « --- » est envoyé tel quel à Claude.
# Cet en-tête, lui, n'est lu que par l'application : il décrit le modèle et
# règle la mise en page du PDF. Tout y est facultatif ; une ligne supprimée
# reprend sa valeur par défaut. Après une modification de « pdf: »,
# le bouton « Refaire le PDF » applique les réglages sans relancer Claude.
...
pdf:
  ...
  titre: "{Thème de la séance}"     # {Champ} = valeur lue dans le tableau d'en-tête du support
  sous_titre: "{Matière} · {Durée}"
  pied_de_page: "{Matière}"
  couleur: "#1e4e8c"                # bandeaux, cartes, tableaux
  niveau_sections: auto             # auto, ou 2 pour les titres « ## », 3 pour « ### »
  nouvelle_page: oui                # chaque grande section commence une page
  cartes: FICHE                     # titres contenant ce mot → cartes insécables (non = aucune)
  reponses: visibles                # visibles, masquees (quiz à remplir) ou fin (corrigé en fin de document)
  sommaire: non
  format: A4                        # A4, A5, A3, Letter, Legal
  orientation: portrait             # portrait ou paysage
  marges: 14mm 14mm 16mm 14mm       # haut droite bas gauche
  taille_texte: 10.5pt
  css: |
    /* CSS libre, appliqué en dernier. Exemple :
       .carte { background: #fbfcff; }
       h4.sous { color: #7a2e0e; } */
---

# Consignes — Cours complet (« Résumés de cours »)
...
Chaque message que je t'envoie dans ce projet contient **la transcription brute d'un cours** (généralement 1 h à 2 h, soit 15 000 à 20 000 mots), et parfois **les supports de ce cours** : diapositives, PDF, photos du tableau, notes. Ton rôle est de les transformer en support de révision exploitable.
...
3. **N'invente jamais** de contenu absent de la transcription et des supports fournis. Si un passage est inaudible ou incohérent, écris `[passage inaudible]` et continue.
...
## Supports du cours (diapositives, photos du tableau, notes)

Quand des supports accompagnent la transcription :

1. **Lis-les tous avant de rédiger** — photos, images et PDF avec l'outil Read.
2. **Croise-les avec la transcription.** L'audio fait foi pour ce qui a été dit et pour ce sur quoi le professeur a insisté ; les supports font foi pour l'orthographe des termes, les noms propres, les sigles, les chiffres, les formules et les schémas. Corrige la transcription grâce à eux sans le signaler.
3. **Complète** les formules et schémas des fiches avec ceux des diapositives et du tableau ; reprends une définition écrite quand elle est plus précise que l'oral.
4. **Cite la source** à côté de l'horodatage : `[00:47 · diapo 12]`, `[01:05 · photo 2]`. L'heure de prise de vue d'une photo aide à la situer dans le cours.
5. **Signale les écarts.** Ce qui figure sur un support sans avoir été abordé en cours va dans les points clés avec la mention *(support seul)*. Une contradiction entre l'oral et un support va en section 10.
6. **Photos du tableau** : insère chaque photo utile dans la fiche ou le thème correspondant, sur sa propre ligne, avec `![Tableau — sujet](supports/nom_du_fichier.jpg)` en reprenant exactement le nom de la liste. Une photo floue ou illisible n'est pas insérée : signale-la en section 10.
7. Sans supports, ignore cette partie.
